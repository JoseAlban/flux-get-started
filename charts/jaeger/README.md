...
## Mutated config
- thrift's tchannel iof grpc for agent-collector
- Elasticsearch won't use password, but AWS signed requests

## Added dependencies
- AWS signing proxy sidecar - to sign all requests to Elasticsearch on AWS
- Elasticsearch Curator (via different chart, instantiated from flux/jaeger template) - to remove older traces
